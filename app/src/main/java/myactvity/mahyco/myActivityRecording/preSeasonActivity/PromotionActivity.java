...
*/
    ...
    public boolean validation() {
        ...
        if (etProgramName.getText().length() == 0) {
            msclass.showMessage("Please enter program name");
            return false;
        }
...
//                        "OTHER"));
    ...
    public void uploadData(String PromotionData) {
        String str = null;
       /* if (config.NetworkConnection()) {
            try {
                new PromotionActivity.UploadPromotionDataServer(PromotionData, context).execute(SERVER).get();
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else*/
...
//            progressDailog.show();
    ...
    public synchronized String syncPromotionDataSingleImage(JSONObject jsonObject) {
        ...
        postParameters.add(new BasicNameValuePair("PromotionData", jsonObject.toString()));
